    ...
    naze.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    naze.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `↜SELAMAT DATANG DI GROUP↝${metadata.subject} @${num.split("@")[0]} SEMOGA BETAH YAH😁\n💌 Intro dulu :\n🏅 nama    : \n𖣔 Umur     :\n➫Asal     :` })
                } else if (anu.action == 'remove') {
                    naze.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} ↜KELUAR DARI GROUP ↝${metadata.subject} \n KALO BALIK LAGI BELIIN MOMOGI😗` })
